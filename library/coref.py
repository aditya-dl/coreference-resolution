...
Markable = namedtuple('Markable',
                      ['string', 'entity', 'start_token', 'end_token', 'tags'])
Document = namedtuple(
    'Document', ['clusters', 'gold', 'mention_to_gold', 'mention_to_cluster'])
...
def get_markables_for_entity(markables, entity):
    ...
    """
    return [
        " ".join(markable.string) for markable in markables
        if markable.entity == entity
    ]
...
def read_data(page_name,
    ...
    markables = sorted([markify(c) for c in xmarkables],
...
def get_entities(markables):
    ...
    C_dict = {
        entity: [
            i for i, markable in enumerate(markables)
            if markable.entity == entity
        ]
