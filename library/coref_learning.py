...
class FFCoref(nn.Module):
    ...
    def __init__(self, feat_names, hidden_dim):
        ...
        self.net = nn.Sequential(nn.Linear(len(self.feat_names), hidden_dim),
                                 nn.Tanh(), nn.Linear(hidden_dim, 1))

    def get_feature_vector_from_feature_map(self, features):
        return ag.Variable(
            torch.FloatTensor(
                [features[feature] for feature in self.feat_names]))
    ...
    def instance_top_scores(self, markables, feats, i, true_antecedent):
        ...
        if i == 0 or i == true_antecedent:
            ...
            all_trues_indices = torch.LongTensor([
                ...
                if markables[index].entity == markables[i].entity
            ])
    ...
    for _ in range(epochs):
        ...
        for doc in markable_set:
            ...
            for i in range(len(doc)):
                max_t, max_f = model.instance_top_scores(
                    doc, feats, i, true_ants[i])
                if max_t is None: continue
                marg_tensor = ag.Variable(torch.Tensor(
                    [margin]))  # this var is not reusable
...
def make_resolver(features, model):
    return lambda markables: [
        utils.argmax(model.score_instance(markables, features, i))
...
def custom_max(vector, skip_index):
    ...
    if not (skip_index == 0 or skip_index == vector.shape[1] - 1):
        vec_ft = torch.cat(
            (vector[0, 0:skip_index], vector[0, skip_index + 1:]), 0)
